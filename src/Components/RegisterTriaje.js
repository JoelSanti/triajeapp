...
import FrmTriaje from "./Forms/FrmTriaje";
...
const RegisterTriaje = () => {
  ...
  const { modalGuardado , switchModalGuardado } = generalsContext;
  const alertContext = useContext(AlertContext);
  const {alerta} = alertContext;

  const cerrarModal = ()=>{
    switchModalGuardado(false)
  }

  return (
    <div className="h-screen py-4 md:py-0 md:pt-28 bg-gradient-to-r from-cyan-500 to-blue-500 relative">
      <div className="flex flex-row flex-wrap justify-center items-center w-11/12 md:w-10/12 border-2 relative mx-auto  p-8 h-full md:h-5/6  rounded-2xl shadow-lg bg-slate-50 ">
      ...
      <div
        className={
          modalGuardado
            ? "absolute w-full h-screen bg-slate-500 top-0 bg-opacity-75 z-20"
            : "hidden"
        }
      >
        <div className="bg-slate-300 w-11/12 md:h-2/6 md:w-1/3 rounded-xl mx-auto mt-60 z-50 opacity-100">
          <div className="px-5 py-6 grid grid-cols-1 gap-6 ">
            <img
              src={checkIcon}
              alt="icon"
              className="h-20  mx-auto motion-safe:animate-pulse"
            ></img>
            <h1 className="text-lg font-medium text-cyan-700 text-center">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              onClick={cerrarModal}
                to="/Listado"
                className="text-center p-4 bg-cyan-600 rounded-lg text-white hover:bg-cyan-700"
              >
                Ver registro
              </Link>
              <button
                onClick={cerrarModal}
                type="button"
                className="p-4 bg-cyan-700 rounded-lg text-white hover:bg-cyan-800"
              >
                Ingresar otro paciente
              </button>
            </div>
          </div>
        </div>
      ...
      {
            alerta ? (
              <div className={`absolute top-6 right-2  p-4 m-4 md:m-0 rounded-lg animate-bounce ${alerta.ctg}`}>
                      {alerta.msg}
              </div>
            ): null
         }
